...
step_size = 200;
for k = 1:300
    for i = 0:step_size:length(X_train)-step_size
        ind_start = i + 1;
        ind_end   = i + step_size;
        if (i==0) ind_start = 1;ind_end = step_size;end
        [J_his(end+1), theta_temp] = nnCostFunction(theta_un, X_train(ind_start:ind_end,:), y_class_train(ind_start:ind_end,:), lambda); % [1x1, (m_train)x10]
        theta_un = theta_un - alpha_rate*theta_temp;
    end
end
toc
