...
% mit: 1/m und ohne sigmoidG für d3 => 91.5%
J = 1/(4*m) * sum(sum( -y_class .* log(h) - (1 - y_class) .* log(1 - h) )) + (lambda / (2 * m)) * (sum(sum(t1 .^ 2)) + sum(sum(t2 .^2 )));

% Backpropagation vectorized

d3 = (h - y_class).* sigmoidG(h);           % 5000 x 10
